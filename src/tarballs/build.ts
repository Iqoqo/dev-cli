...
const pack = async (from: string, to: string) => {
  ...
  log(
    `packing tarball from ${qq.prettifyPaths(from)} to ${qq.prettifyPaths(to)}`
  );
...
export async function build(
  ...
  const prevCwd = qq.cwd();
  const packCLI = async () => {
    const stdout = await qq.x.stdout("yarn", ["pack", "--unsafe-perm"], {
      cwd: c.root
    });
  ...
  const addDependencies = async () => {
    ...
    if (yarn) {
      ...
      await qq.x("yarn install --production");
    }
  };
  const buildTarget = async (target: {
    ...
    if (xz)
      await pack(
        workspace,
        c.dist(config.s3Key("versioned", ".tar.xz", target))
      );
    if (!c.updateConfig.s3.host) return;
    const rollout =
      typeof c.updateConfig.autoupdate === "object" &&
      c.updateConfig.autoupdate.rollout;
    const manifest: IManifest = {
      ...
      xz: xz
        ? config.s3Url(config.s3Key("versioned", ".tar.xz", target))
        : undefined,
      sha256gz: await qq.hash(
        "sha256",
        c.dist(config.s3Key("versioned", ".tar.gz", target))
      ),
      sha256xz: xz
        ? await qq.hash(
            "sha256",
  ...
  const buildBaseTarball = async () => {
    ...
    if (!c.updateConfig.s3.host) {
      Errors.warn(
        "No S3 bucket or host configured. CLI will not be able to update."
      );
      return;
    }
    const manifest: IManifest = {
      ...
      sha256gz: await qq.hash(
        "sha256",
        c.dist(config.s3Key("versioned", ".tar.gz"))
      ),
      sha256xz: xz
        ? await qq.hash("sha256", c.dist(config.s3Key("versioned", ".tar.xz")))
        : undefined,
      rollout: (typeof c.updateConfig.autoupdate === "object" &&
        c.updateConfig.autoupdate.rollout) as number,
      node: {
        compatible: config.pjson.engines.node,
        recommended: c.nodeVersion
      }
